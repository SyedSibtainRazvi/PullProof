...
The user message includes YOUR PREVIOUS REVIEW of an earlier revision of this post; the author has updated it since. Review incrementally:
- Stay consistent with your previous feedback — do not flip a rating or contradict earlier advice without explicitly saying what changed.
- Start Issues Found with a single 'Resolved since last review:' line listing previously-raised issues the author has addressed (titles only), then number only new or still-unresolved issues.
- Do not re-raise resolved items, and do not invent new nitpicks to fill space when the update addressed your feedback."
  fi

  local escaped_prompt=$(jq -n --arg sys "$system_prompt" --arg file "$filename" --arg content "$content" --arg facts "$facts" --arg prev "$previous_review" \
    '{"role": "system", "content": $sys}, {"role": "user", "content": ("File: " + $file + "\n\n" + $facts + (if $prev != "" then "\n\nYOUR PREVIOUS REVIEW (of an earlier revision):\n" + $prev else "" end) + "\n\nContent to review:\n" + $content)}' |
...
# Fetch our previous review (if any) up front: its body is fed back into the
# prompt so successive reviews stay consistent across iterations, and its id
# is reused when posting so the comment list isn't fetched twice.
EXISTING_COMMENT=$(curl -sf --connect-timeout 10 --max-time 30 \
  -H "Authorization: token $GITHUB_TOKEN" \
  -H "Accept: application/vnd.github.v3+json" \
  "https://api.github.com/repos/$REPO/issues/$PR_NUMBER/comments?per_page=100" \
  | jq -c "[.[] | select(.body | startswith(\"$COMMENT_MARKER\"))][0] // {}" || true)

EXISTING_COMMENT_ID=$(echo "$EXISTING_COMMENT" | jq -r '.id // empty' 2>/dev/null)
PREVIOUS_REVIEW=$(echo "$EXISTING_COMMENT" | jq -r '.body // empty' 2>/dev/null)

if [ -n "$PREVIOUS_REVIEW" ]; then
  echo "Found previous review (comment $EXISTING_COMMENT_ID) — reviewing incrementally."
fi
...
for i in $(seq 0 $((FILE_COUNT - 1))); do
  ...
  REVIEW=$(get_review_content "$FILENAME" "$CONTENT" "$FACTS" "$PREVIOUS_REVIEW")
